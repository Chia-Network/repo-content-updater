...
func TestProcessTemplateOverrides(t *testing.T) {
	...
	result, err := repo.ProcessTemplate(template, map[string]interface{}{"CGO_ENABLED": "0"}, map[string]interface{}{})
	assert.Nil(t, err)
	assert.Equal(t, string([]byte(fmt.Sprintf("%d 0", time.Now().Year()))), string(result))

	// Ensure allowed overrides work
	result, err = repo.ProcessTemplate(template, map[string]interface{}{}, map[string]interface{}{"CGO_ENABLED": "1"})
	assert.Nil(t, err)
	assert.Equal(t, []byte(fmt.Sprintf("%d 1", time.Now().Year())), result)

	// Ensure disallowed overrides dont override
	result, err = repo.ProcessTemplate(template, map[string]interface{}{}, map[string]interface{}{
		"CGO_ENABLED":  "1",
		"CURRENT_YEAR": "1990",
	})
	assert.Nil(t, err)
